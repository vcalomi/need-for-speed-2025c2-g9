...
std::shared_ptr<GameRoom> GameMonitor::getRoomByClient(int clientId) {
    ...
    if (!clientToRoom.count(clientId)) return nullptr;
    return clientToRoom[clientId];
}

bool GameMonitor::createGameRoom(const std::string& roomName, int hostId, Queue<std::shared_ptr<Dto>>& hostQueue, int maxPlayers) {
    std::lock_guard<std::mutex> lock(mtx);
...
bool GameMonitor::joinGameRoom(const std::string& roomName, int clientId, Queue<std::shared_ptr<Dto>>& clientQueue) {
    std::lock_guard<std::mutex> lock(mtx);
...
bool GameMonitor::startGameByClientId(int clientId) {
    ...
    
    {
        ...
        if (!room->isHost(clientId)) return false;
    ...
    for (auto& [clientId, callback] : callbacksToExecute) {
        try {
            if (callback && targetRoom) {
                callback(targetRoom);
            }
        } catch (const std::exception& e) {
            std::cout << "Error in callback for client " << clientId << ": " << e.what() << std::endl;
        }
    }
...
void GameMonitor::registerStartNotifier(int clientId, std::function<void(std::shared_ptr<GameRoom>)> notifier) {
    std::lock_guard<std::mutex> lock(mtx);

...
static std::optional<int> extract_suffix_index(const std::filesystem::path& p) {
    ...
    if (pos == std::string::npos || pos + 1 >= stem.size()) return std::nullopt;
    std::string suf = stem.substr(pos + 1);         // "2"
    for (unsigned char c : suf) if (!std::isdigit(c)) return std::nullopt;
    try { return std::stoi(suf); } catch (...) { return std::nullopt; }
}
...
Matrix LevelCreator::BuildLevelMatrix(const std::filesystem::path& file_path) {
    ...
    for (const auto& layer : J["layerInstances"]) {
        ...
        if (layer.value("__type", "") != "IntGrid") continue;
        if (layer.value("__identifier", "") == "Collision2px") { L = &layer; break; }
    }
    if (!L) { std::cerr << "[ERROR] 'Collision2px' not found in " << file_path << '\n'; return {}; }

    const int cw = L->value("__cWid", 0);
    const int ch = L->value("__cHei", 0);
    if (cw <= 0 || ch <= 0) { std::cerr << "[ERROR] invalid grid dims in " << file_path << '\n'; return {}; }
...
void LevelCreator::processDirectoryLevel(const std::string& directory_path) {
    ...
    for (const auto& entry : std::filesystem::directory_iterator(directory_path, ec)) {
        if (ec) { std::cerr << "[ERROR] iterating " << directory_path << ": " << ec.message() << '\n'; break; }
        if (!entry.is_regular_file() || entry.path().extension() != ".ldtkl") continue;

        auto idxOpt = extract_suffix_index(entry.path());
        if (!idxOpt) continue;           // sin sufijo numérico => salteamos

        int idx = *idxOpt;
        if (idx < 0 || idx >= 4) continue; // fuera del rango 0..3 => salteamos
